...
*/
public class MakePDBSeqadv35Debug_JMC {
    ...
    static class DBRef {
        ...
        public String toString() {
            return ("DBRef: "+dbName+" "+dbCode+" "+id+" "+entity+" "+chain+" "+pdbStart+" "+pdbEnd);
        }
    }
    ...
    static class SEQADV {
        ...
        //190130: added toString for debugging
        public String toString() {
            return ("SEQADV: "+category+" "+pdbResID+" "+seq+" "+pdbStart+" "+pdbEnd+" "+dbRef);
        }
    }
    ...
    static class PDBMLHandler extends DefaultHandler {
        ...
        public void startElement(String uri,
            ...
            // YuC: to fix pdbStart insertion code
            else if ((qName.equals("PDBx:pdbx_seq_align_beg_ins_code"))
                && (inElement==2)) {
                System.out.println("enter the beg_ins_code");
                System.out.println("current inElement: " + inElement);
                inElement = 11;
                //curDBRef.pdbStart = "";
            }
        ...
        public void endElement(String uri,
            ...
            if (qName.equals("PDBx:struct_ref")) {
                ...
                if (curChain.length()==1)
                    curDBRef.chain = curChain.charAt(0);
                System.out.println("debug3: ref="+curDBRef.toString());
                if (curDBRef.entity == null)
                    System.out.println("DBRef found without entity");
                else {
                    if (curDBRef.ok()) {
                        DBRef saveDBRef = new DBRef(); // make a copy to save
                        saveDBRef.merge(curDBRef);

                        if (dbRefs.get(saveDBRef.entity)==null)
                            dbRefs.put(saveDBRef.entity, new Vector<DBRef>());
                        if (!dbRefs.get(saveDBRef.entity).contains(saveDBRef)) {
                            dbRefs.get(saveDBRef.entity).add(saveDBRef);
                        }
                        System.out.println("debug4: curAlignID: " + curAlignID);
                        System.out.println("debug4: saveDBRef: " + saveDBRef);
                        alignIDMap.put(curAlignID, saveDBRef);
                        System.out.println("debug4: alignIDMap: " +  alignIDMap);
                        System.out.println("debug4: dbRefs: " + dbRefs);
                    }
                }
            }
            else if (qName.equals("PDBx:struct_ref_seq_dif")) {
                ...
                tmpStr = null;
                     ...
                     (inElement==10)) {
                inElement = 2;
                System.out.println("debug5: tmpStr for redIDMap: " + tmpStr);
                DBRef savedDBRef = refIDMap.get(tmpStr);
                if (savedDBRef == null)
                    System.out.println("unknown ref_id "+tmpStr);
                else {
                    System.out.println("debug5: savedDBRef before merge: " + savedDBRef);
                    System.out.println("debug5: curDBRef before merge: " + curDBRef);
                    savedDBRef.merge(curDBRef);
                    curDBRef = savedDBRef;
                    System.out.println("debug5: curDBRef after merge: " + curDBRef);
                    System.out.println("debug5: savedDBRef after merge: " + savedDBRef);
                }
                tmpStr = null;
            }
          //YuC: edits
        ...
        public void characters(char[] ch,
            ...
            else if ((inElement==17) || (inElement==19))
                ...
                if (rs.next()) {
                    ...
                    if (dbRefs != null) {
                        for (DBRef d : dbRefs) {
                            ...
                            //System.out.println("debug: "+d.chain);
                            if ((d.chain=='*') ||
                                (d.chain==chain)) {
                                ...
                                System.out.println("dbStart: " + d.dbStart);
                                System.out.println("dbEnd: " + d.dbEnd);
                    ...
                    // 190129: where issues are: null pointer exception
                    ...
                    if (seqadvs != null)
                        ns = seqadvs.size();
                        System.out.println("seqadvs: " + seqadvs);
                    SEQADV s1 = null;
                    for (int j=0; j<=ns; j++) {
                        SEQADV s2 = null;
                        if (j<ns) {
                            ...
                            //190129: questions on this line, why start and end the same?
                        ...
                        if ((s1 != null) && (s2 != null)) {
                            System.out.println("check enter:a");
                        ...
                        if (!merged) {
                            System.out.println("check enter:b");
                            if (s1 != null) {
                                // save s1
                                System.out.println("check enter:c");
                                System.out.println("check enter:c " + s1.dbRef);
                                System.out.println("check enter:c " + s1.category);
                                System.out.println("check enter:c " + s1.pdbStart);
                                System.out.println("check enter:c " + s1.pdbEnd);
                                System.out.println("check enter:c " + s1.seq);
                                System.out.println("check enter:c1");
                                System.out.println("assignedIDs:" + assignedIDs);
                                if (assignedIDs.get(s1.dbRef)!=null) {
                                    stmt3.setInt(2,assignedIDs.get(s1.dbRef));
                                    System.out.println("check enter:c2");
                                    stmt3.setInt(3,lookupOrCreateCategory(s1.category));
                                    System.out.println("check enter:c3");
                                    stmt3.setInt(4,s1.pdbStart);
                                    stmt3.setInt(5,s1.pdbEnd);
                                    stmt3.setString(6,s1.seq);
                                    if ((s1.pdbStart > -1) &&
                                        (s1.pdbEnd > -1)){
                                        System.out.println("check enter:c4");
                                        stmt3.executeUpdate();}
                                }
                            }
                            System.out.println("check enter:d");
                            // if (s2 != null) {
                            //     System.out.println("check s2 pdbStart:" + s2.pdbStart);
                            //     System.out.println("check s2 pdbEnd:" + s2.pdbEnd);
                            // }
                            s1 = s2;
                        }
                    }
